...
import { getQuizzes, createQuiz, updateQuiz, deleteQuiz, publishQuiz, unpublishQuiz } from "@/api/supabase/quizzes"
import type { Quiz } from "@/lib/types"
import { useAuth } from "@/lib/auth"

// Hook to fetch all quizzes with pagination
export const useQuizzes = (page = 1, pageSize = 9, isPublicRoute = false) => {
  const { user } = useAuth()

  return useQuery({
    queryKey: ["quizzes", { page, pageSize }],
    queryFn: () => getQuizzes(page, pageSize, isPublicRoute),
...
export const useUpdateQuiz = () => {
  ...
  return useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: Partial<Quiz> }) => updateQuiz(id, updates),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["quizzes"] })
...
export const useTogglePublishQuiz = () => {
  ...
  return useMutation({
    mutationFn: ({ id, publish }: { id: string; publish: boolean }) => (publish ? publishQuiz(id) : unpublishQuiz(id)),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["quizzes"] })
      queryClient.invalidateQueries({ queryKey: ["quiz", data.id] })
    },
